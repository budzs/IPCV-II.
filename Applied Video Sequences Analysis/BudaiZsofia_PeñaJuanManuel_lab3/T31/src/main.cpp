...
int main(int argc, char ** argv)
{
	...
	std::string inputvideo = "/home/avsa/AVSA_Lab3_datasets/dataset_lab3/lab3.1/singleball.mp4"; 	// path for the video to process
	...
	if (!cap.isOpened())
	 ...
	 Ptr<BackgroundSubtractor> bgsubtractor = cv::createBackgroundSubtractorMOG2(50, 60.0, true);
...
/*		//preload measurements from txt file
	...
	for (;;) {
		...
		//cv::morphologyEx(fgmask,opened_fgmask,MORPH_CLOSE, getStructuringElement(MORPH_RECT, Size(2,2))); // apply morphological opening

		cv::morphologyEx(fgmask,opened_fgmask,MORPH_OPEN, getStructuringElement(MORPH_RECT, Size(2,2))); // apply morphological opening
		//cv::morphologyEx(fgmask,opened_fgmask,MORPH_CLOSE, getStructuringElement(MORPH_RECT, Size(4,6))); // apply morphological opening
...
//		}
		...
		ShowManyImages(title, int(4), frame, opened_fgmask, paintBlobImage(b_frame,bloblistFiltered, false),  img_kalman);
