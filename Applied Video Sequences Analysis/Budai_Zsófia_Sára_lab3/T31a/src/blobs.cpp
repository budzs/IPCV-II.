...
int getBiggestBlob(std::vector<cvBlob> bloblist_in, std::vector<cvBlob> &bloblist_out){
	// select the biggest blob for the frame and place it in bloblist_out
	bloblist_out.clear();

	// if there is no blob detected return
	if (bloblist_in.size() == 0){
		return 1;
	}

	// initialize the biggest with the first frame's blob
	cvBlob Biggest = bloblist_in[0];

	// max search for blob size
	for(unsigned int i = 0; i < bloblist_in.size(); i++)
	{
		cvBlob blob_in = bloblist_in[i]; //get ith blob

		int size = blob_in.h * blob_in.w;
		int size_biggest = Biggest.h * Biggest.w;
	...
	// place the biggest blob into the bloblist_out
	bloblist_out.push_back(Biggest);
	return 1;
}
