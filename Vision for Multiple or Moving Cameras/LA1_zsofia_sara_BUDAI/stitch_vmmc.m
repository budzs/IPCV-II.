function [stitched_image, stitched_mask, im1, im2] = stitch_vmmc(im1, im2, T_im2, mask)
% STITCH_VMMC Given two images im1 and im2 and a transformation T_im2 that 
% shall be applied to im2, this function computes a composite image and 
% a mask. The mask 'stitched_mask' is a matrix of the same dimensions as 
% the resulting image whose pixel values are the number of image layers on
% each pixel. Upon subsequent calls, previously calculated masks have to be
% fed back using the mask parameter.
%
%
% Example of usage:
%   H = homography_auto_iavs(im1, im2, 1);  
%   im_TH = maketform('projective',H');   
%   [im_stitchedH, stitched_maskH, im1TH, im2TH] = stitch(im2, im1, im_TH);      
%
%
