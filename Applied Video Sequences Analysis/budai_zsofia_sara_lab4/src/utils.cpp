...
std::vector<double> compareCandidates(const cv::Rect& objectModel, const std::vector<cv::Rect>& candidates, const cv::Mat& frame) {
	    std::vector<double> distances;

	    // object model region from the image and convert it to grayscale
	    cv::Mat modelRegion = frame(objectModel);
	    cv::Mat grayObjectModel;
	    cv::cvtColor(modelRegion, grayObjectModel, cv::COLOR_BGR2GRAY);

	    for (const cv::Rect& candidate : candidates) {
	        // candidate region from image and convert it to grayscale
	        cv::Mat candidateRegion = frame(candidate);
	        cv::Mat grayCandidate;
	        cv::cvtColor(candidateRegion, grayCandidate, cv::COLOR_BGR2GRAY);

	        // resizing
	        cv::Mat resizedObjectModel;
	        cv::resize(grayObjectModel, resizedObjectModel, grayCandidate.size(), 0, 0, cv::INTER_LINEAR);

	        // the template matching
	        cv::Mat result;
	        cv::matchTemplate(grayCandidate, resizedObjectModel, result, cv::TM_CCOEFF_NORMED);
	        ...
	        // convert to a distance metric and store in the returning list
	        double distance = 1.0 - maxVal;
