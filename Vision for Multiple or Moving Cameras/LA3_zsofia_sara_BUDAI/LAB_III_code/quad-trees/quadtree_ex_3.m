...
clc
%% parameters (example)
K     = 11;                             % maximum number of 'levels'
alpha = 0.035;                         % threshold / condition
fun   = @(x) (max(max(x))-min(min(x))) > alpha; % driving function: variance > alpha
%% read some image and conver to double [0,1]
imargb  = im2double(imread('ima_T1.jpg'));
%% resize to 2.^K, 2.^K
