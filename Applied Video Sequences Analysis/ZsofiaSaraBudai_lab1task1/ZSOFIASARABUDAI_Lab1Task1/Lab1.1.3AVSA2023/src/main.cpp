...
//namespaces
using namespace std;
//to avoid using'std' to declare std functions and variables (e.g., std::out -> out)
using namespace cv;
//to avoid using 'cv' to declare OpenCV functions and variables (e.g., cv::Mat -> Mat)
...
//
	string dataset_path = "/home/zsofi/Downloads/AVSA/AVSASlidesVideos_dataset"; //SET THIS DIRECTORY according to your download
...
// if (argc==3)	{input=argv[3]);}
	...
	string project_root_path = "/home/zsofi/eclipse-workspace/AVSA2020results/"; //SET THIS DIRECTORY according to your project
	string project_name = "Lab1.1.3AVSA2023"; // project exe name
...
//			namedWindow("ForeGround Mask"); // final fgmask
			...
			double tau = 25.0; // to set ... //Lab1.1.1-3
			double alpha = 0.12; // to set ... //Lab1.1.2-3
			...
			fgseg::bgs avsa_bgs(tau, alpha, threshold_ghosts2, rgb); //construct object of the bgs class
...
//		        imshow("ForeGround Mask", fgmask);
				...
				if (selective_bkg_update)
					selective_flag = "select";
				else
					selective_flag = "blind";
				...
				string title =
						project_name
								+ " | Frame - Bg - Bg0 - 10BgDiff || Diff - BgSM - ShwM - FgM - | ("
								+ dataset_cat[c] + "/" + baseline_seq[s] + " "
								+ color_flag + "; tau=" + tau_ss.str() + "; "
								+ selective_flag + "; alpha= " + alpha_ss.str()
								+ "; th_gh2= " + th_gh2_ss.str() + ")";

				Mat bg_diff;
				absdiff(bg, bg0, bg_diff);
				bg_diff *= 10; // to see better the diff

				ShowManyImages(title, 8, img, bg, bg0, bg_diff, diff, bgsmask,
						shdmask, fgmask);

				//save results of your bgs algorithm
				//...
				string outFile = results_path + "/" + dataset_cat[c] + "/"
						+ baseline_seq[s] + "/" + color_flag + tau_ss.str()
						+ selective_flag + alpha_ss.str() + th_gh2_ss.str()
						+ "out" + it_ss.str() + ".png";
...
//		       	if (_CONSOLE_DEBUG){cout << outFile << endl;}
				...
				if (!write_result)
					printf("ERROR: Can't save fg mask.\n");
